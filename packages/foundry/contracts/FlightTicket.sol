...
contract FlightTicket is ERC1155, Ownable {
    ...
    constructor(address _initialOwner) ERC1155("https://localhost:5626/ticket/{id}") Ownable(_initialOwner) {}
    ...
    external onlyOwner {
        ...
        if (_departureTime <= block.timestamp + 1 days) revert FlightTicket_FlightCannotBeLessThanOneDay(_departureTime, block.timestamp);
        if (bytes(_aircraftModel).length == 0) revert FlightTicket_AircraftCannotBeEmpty(_aircraftModel);
        ...
        emit FlightTicket_FlightCreated(s_flightId, _airportOrigin, _airportDestination, _departureTime, _aircraftModel, _totalSeats, price);

        s_flights[s_flightId] = Flight(_airportOrigin, _airportDestination, _departureTime, _aircraftModel, _totalSeats, price, 0, 0);
    ...
    function bookSeat(uint256 _flightId) external payable {
        ...
        if (flight.seatsBooked >= flight.totalSeats) revert FlightTicket_NoSeatsAvailable(_flightId, flight.seatsBooked, flight.totalSeats);
        if (msg.value > flight.price || msg.value < flight.price) revert FlightTicket_IncorrectPaymentAmount(_flightId, msg.value, flight.price);
    ...
    function getFlightBalance(uint256 flightId) external view onlyOwner returns (uint256) {
        require(flightId < s_flightId, "Flight does not exist");
        return s_flights[flightId].balance;
    }

    function withdrawFlightFunds(uint256 flightId) external onlyOwner {

        require(flightId < s_flightId, "Flight does not exist");
        require(s_flights[s_flightId].departureTime < block.timestamp, "Flight hour");

        Flight storage flight = s_flights[flightId];

        uint256 amount = flight.balance;
        flight.balance = 0;

        emit FlightTicket_FundsWithdrawn(flightId, amount);
